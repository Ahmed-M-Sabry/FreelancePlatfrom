...
{
    public class RestoreSkillHandler : ResponseHandler, IRequestHandler<RestoreSkillCommand, ApiResponse<string>>
    {
        private readonly UserManager<ApplicationUser> _userManager;
        private readonly IHttpContextAccessor _httpContextAccessor;
        private readonly ISkillService _skillService;
        private readonly IMapper _mapper;
        public RestoreSkillHandler(UserManager<ApplicationUser> userManager
        ...
        public async Task<ApiResponse<string>> Handle(RestoreSkillCommand request, CancellationToken cancellationToken)
        {
            ...
            if (isSkillExisted == null)
                return NotFound<string>($"Skill Not Found With Is {request.Id}");

            if (!isSkillExisted.IsDeleted)
                return BadRequest<string>($"Skill : {request.Id} Is Not Deleted");

            await _skillService.RestoreSkill(request.Id);

            return Created<string>($"Skill With id = {request.Id} is Restored");
        }
    }
}
