...
{
    public class RestoreCountryHandler : ResponseHandler
        , IRequestHandler<RestorCountryCommand, ApiResponse<string>>
    {
        private readonly UserManager<ApplicationUser> _userManager;
        private readonly IHttpContextAccessor _httpContextAccessor;
        private readonly ICountryServices _countryServices;
        private readonly IMapper _mapper;
        public RestoreCountryHandler(UserManager<ApplicationUser> userManager
        ...
        public async Task<ApiResponse<string>> Handle(RestorCountryCommand request, CancellationToken cancellationToken)
        {
            ...
            if (!country.IsDeleted)
                return BadRequest<string>($"Country with Id '{request.Id}' is not deleted.");

            country.IsDeleted = false;
            await _countryServices.UpdateCountry(country);

            return Created<string>($"Country with Id '{request.Id}' has been restored.");
        }
    }
}
