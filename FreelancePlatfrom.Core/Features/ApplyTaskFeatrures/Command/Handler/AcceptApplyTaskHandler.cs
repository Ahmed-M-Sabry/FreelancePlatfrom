...
{
    public class AcceptApplyTaskHandler : ResponseHandler, IRequestHandler<AcceptApplyTaskCommand, ApiResponse<string>>
    {
        ...
        public AcceptApplyTaskHandler(UserManager<ApplicationUser> userManager,
        ...
        public async Task<ApiResponse<string>> Handle(AcceptApplyTaskCommand request, CancellationToken cancellationToken)
        {
            ...
            if (!isClient)
                return BadRequest<string>("You Must Be a Client to Accept This Apply Task");
            ...
            if (existingApplyTask is null)
                return BadRequest<string>("Apply Task Not Found To Accept");
            ...
            var acceptApplyTast =  await _applyTaskService.AcceptApplyTask(userId, request.Id);
            if(acceptApplyTast.Status != ApplyTaskStatus.Accepted)
                return BadRequest<string>("Some thing Error Happen Try Again");

            return Success<string>("Apply Task Accepted Successfully");
        }
    }
}
